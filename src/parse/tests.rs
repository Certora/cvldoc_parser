...
use crate::parse::types::{Ast, Token};
...
fn new_parser() {
    ...
    */
    ...
    "#};
    let mut after_lexing = crate::parse::lexer().parse(src).unwrap();
    let len = src.chars().count();

    after_lexing
        .retain(|(tok, _span)| !matches!(tok, Token::SingleLineComment | Token::MultiLineComment));
    ...
    let (after_parsing, errors) = parser().parse_recovery(stream);
    let mut parsed = after_parsing.unwrap();
    // parsed.retain(|(ast, _span)| !matches!(ast, Ast::ParseError));

    let builder = Builder::new(src);

    for (ast, span) in parsed {
        if let Ast::ParseError = ast {
            continue;
        };
        let processed = builder.process((ast, span));
        dbg!(processed);
    }
